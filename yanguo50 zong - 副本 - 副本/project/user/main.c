...
//	}
	...
	//		pid.Speed_KP_L = pid.Speed_KP_R = pid.Speed_KP_R + 0.1;
		...
		if(mt9v03x_finish_flag)
		{
			qy_time = 0;
//			printf("%d\n",qy_time);
			// ============ Ë«»º³åDMA£ºÖ¸ÕëÖ±½ÓÖ¸Ïò°²È«»º³åÇø£¬Áã¿½±´ ============
			{
				uint8 done_snapshot = dma_done_sel;  // ¿ìÕÕ£¬·ÀÖ¹ISRÔÚÅÐ¶ÏÆÚ¼äÐÞ¸Ä
				if(done_snapshot == 0)
				{
					// mt9v03x_image¸Õ±»DMAÐ´Íê£¬DMAÏÖÔÚÔÚÐ´image_dma_buf2
					image_copy_out = (image_copy_out_ptr_t)mt9v03x_image;
				}
				else
				{
					// image_dma_buf2¸Õ±»DMAÐ´Íê£¬DMAÏÖÔÚÔÚÐ´mt9v03x_image
	...
	//			LowerCameraExposure();
			...
			if(count1 > 2000)
			{
				Black_counts_weight(80);      //ï¿½ï¿½ï¿½ß±ï¿½ï¿½ï¿½
			}
