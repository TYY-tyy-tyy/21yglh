...
//    }
    ...
    // ==================== 双缓冲DMA方案 ====================
    // 第一步：翻转DMA目标选择 (0→1 或 1→0)
    dma_target_sel = !dma_target_sel;

    // 第二步：调库函数复位FIFO、使能DMA
    mt9v03x_vsync_handler();

    // 第三步：写入DMA地址寄存器（在库调用之后，若有覆盖则重写）
    //         DMA使能后到摄像头开始输出第一行像素有几微秒余量，足够完成
    {
        unsigned long addr;
        uint16 dma_addr;

        if(dma_target_sel == 0)
            addr = (unsigned long)mt9v03x_image;
        else
            addr = (unsigned long)image_dma_buf2;

        dma_addr = (uint16)(addr & 0xFFFF);
        DMA_LCM_RXAL = (uint8)(dma_addr & 0xFF);
        DMA_LCM_RXAH = (uint8)((dma_addr >> 8) & 0xFF);
    }
}
...
void DMA_LCM_IRQHandler() interrupt DMA_LCM_VECTOR
{
    ...
    // ==================== 双缓冲DMA方案 ====================
    // DMA传输刚完成，dma_target_sel就是本次写入的目标缓冲区
    // 记录之，主循环据此设置 image_copy_out 指针
    dma_done_sel = dma_target_sel;
}
