...
void get_reference_point(void)
{
    ...
    //限幅
    white_max_point = (uint8)func_limit_ab((int32)reference_point * WHITEMAXMUL / 10, BLACKPOINT, WHITEPOINT); //平均值 * 130%
    white_min_point = (uint8)func_limit_ab((int32)reference_point * WHITEMINMUL / 10, BLACKPOINT, WHITEPOINT); //平均值 * 70%
...
void search_reference_col(void)
{
	...
	/* 右圆环特殊扫线 */
	if(Find_Right_FLAG == Right_5)
	{
		/* 寻找最长白列 */
		for(col = 94; col < (MT9V03X_W - 20); col += 4)
		{
			if(qqy == 0)
			{
				for(row = MT9V03X_H - 4; row > 0; row -= 4)
				{
					//获取当前灰度值
					gray_point_1 = image_copy_out[row][col];
					//获取对比度灰度值
					gray_point_2 = image_copy_out[row - 4][col];
...
//						image_copy_out[row][col] = 0;
					...
					{
//						image_copy_out[row][col] = 0;
						remote_distance[col] = (uint8)row+2;
						qqy = 1;
						break;
					}
				}
...
//							image_copy_out[row][col] = 0;
	...
	{
		for(col = 20; col < (MT9V03X_W - 94); col += 4)
		{
			if(qqy == 0)
			{
				for(row = MT9V03X_H - 4; row > 0; row -= 4)
...
//							image_copy_out[row][col] = 0;
	...
	else
	{
		for(col = 20; col < (MT9V03X_W-20); col += 2)
		{
			if(qqy == 0)
			{
				for(row = MT9V03X_H - 4; row > 2; row -= 2)
				{
					//获取当前灰度值
					gray_point_1 = image_copy_out[row][col];
					//获取对比度灰度值
