...
void Find_Boundry_LongWhiteCol(void)
{
	...
	/* 根据参考列扫线 */
	for(i = MT9V03X_H - 2; i >= reference_col_farthest; i-=2)
	{
		//中线往右边扫
		if(r_qqy == 0)
		{
			for(j = reference_col; j < MT9V03X_W - 6; j+=2) //91
			{
				//获取当前灰度值
...
//					image_copy_out[i][j] = 0;
				...
				compare_value1 = (gray_point_1 - gray_point_2) * 200 / (gray_point_1 + gray_point_2);
				compare_value2 = (gray_point_1 - gray_point_3) * 200 / (gray_point_1 + gray_point_3);
//				compare_value3 = (gray_point_1 - gray_point_4) * 200 / (gray_point_1 + gray_point_4);
//				compare_value4 = (gray_point_3 - gray_point_4) * 200 / (gray_point_3 + gray_point_4);
				if(compare_value1 > REFERENCE_CONTRAST && compare_value2 > REFERENCE_CONTRAST)
//					&& compare_value3 > REFERENCE_CONTRAST && compare_value4 < REFERENCE_CONTRAST)
				{
//					image_copy_out[i][j] = 0;
					r_qqy = 1;
					right_border = j+2;
...
//						image_copy_out[i][j] = 0;
			...
			{
				for(j = reference_col; j < MT9V03X_W - 8; j+=2) //91
				{
					//获取当前灰度值
					gray_point_1 = image_copy_out[i][j];
					//获取对比度灰度值
...
//						image_copy_out[i][j] = 0;
	...
	/* 元素部分 */
    Find_Ring();
	if((Find_Right_FLAG == Right_0) && (Find_Left_FLAG == Left_0)) Find_Crossroads();

    //根据前面的元素判断传入改变后的左右边界的x坐标，从而二次得到真实的中点坐标
	for(i = MT9V03X_H - 3; i >= reference_col_farthest; i-=2)
	{
