...
//			image_copy_out[row][col] = 0;
			...
			compare_value = (int32)(gray_point_1 - gray_point_2) * 200 / (gray_point_1 + gray_point_2);
...
void Find_Target2(int p1,int p2)
{
	...
	if(Black_p[4] >= 2)
	{
		...
		if(Find_Target_time >= 0)
		{
			...
			if(pos_votes[3] >= 2)           { all_off(); laser_on(LASER_PIN_3); }
			else if(pos_votes[2] >= 2)      { all_off(); laser_on(LASER_PIN_2); }
			else if(pos_votes[4] >= 2)      { all_off(); laser_on(LASER_PIN_4); }
			else if(pos_votes[1] >= 2)      { all_off(); laser_on(LASER_PIN_1); }
			else if(pos_votes[5] >= 2)      { all_off(); laser_on(LASER_PIN_5); }
		}
	}
	else
	{
